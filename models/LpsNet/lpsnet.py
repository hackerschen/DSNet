...
class LPSNet2Path(BaseSegNet):
    ...
    def forward(self, x):
        ...
        x_out = self.head(x_cat)
        x_out = F.interpolate(x_out, x.shape[-2:], mode='bilinear', align_corners=True)
        return x_out
...
def get_lpsnet(depth, width, resolution, num_classes=1, deploy=False):
