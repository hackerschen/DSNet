...
class DualResNet(nn.Module):
    ...
    def __init__(self, criterion, block, layers, num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=False):
    ...
    def forward(self, x, gt=None):
        ...
        x_ = F.interpolate(x_, x_shape[-2:], mode='bilinear', align_corners=True)

        if gt is not None:
            x_extra = self.seghead_extra(temp)
            x_extra = F.interpolate(x_extra, x_shape[-2:], mode='bilinear', align_corners=True)
            main_loss = self.criterion(x_, gt)
            aux_loss = self.criterion(x_extra, gt)
            return main_loss + 0.4 * aux_loss, x_
        else:
            return x_      

def DualResNet_imagenet(criterion, pretrained=False, augment=False):
    model = DualResNet(criterion, BasicBlock, [3, 4, 6, 3], num_classes=1, planes=64, spp_planes=128, head_planes=256, augment=augment)
    if pretrained:
        checkpoint = torch.load('/data/cj/pretrained/' + "DDRNet39_imagenet.pth", map_location='cpu')
        '''       
...
def get_ddr_39(criterion, pretrained=False, augment=True):

    model = DualResNet_imagenet(criterion, pretrained=pretrained, augment=augment)
    return model
