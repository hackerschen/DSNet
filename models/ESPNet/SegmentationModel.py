...
from models.ESPNet.Model import EESPNet, EESP

from models.ESPNet.cnn_utils import BR, CBR, PSPModule, C


class EESPNet_Seg(nn.Module):
    def __init__(self, classes=1, s=1, criterion=None, pretrained=None, gpus=1):
        super().__init__()
    ...
    def forward(self, input, gt=None):
        ...
        if gt is not None:
            output1 = self.hierarchicalUpsample(proj_merge_l3_bef_act)
            loss1 = self.criterion(output, gt)
            loss2 = self.criterion(output1, gt)
