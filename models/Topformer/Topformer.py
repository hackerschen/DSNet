...
from models.Seaformer.Seaformer import resize

from models.Topformer.SimpleHead import SimpleHead
...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        token_head = H
        qq = self.to_q(x).reshape(B, self.num_heads, self.key_dim, token_head, H * W // token_head).permute(0, 1, 3, 4,
                                                                                                            2)
        kk = self.to_k(x).reshape(B, self.num_heads, self.key_dim, token_head, H * W // token_head).permute(0, 1, 3, 2,
                                                                                                            4)
        vv = self.to_v(x).reshape(B, self.num_heads, self.d, token_head, H * W // token_head).permute(0, 1, 3, 4, 2)
        ...
        xx = xx.permute(0, 1, 4, 2, 3).reshape(B, self.dh, H, W)
...
class TopFormer_Seg(nn.Module):
    def __init__(self, type='base',pretrained=False):
        super().__init__()
        if type == 'small':
            self.backbone = Topformer(
                cfgs=model_cfgs_small['cfg'],
                channels=model_cfgs_small['channels'],
                out_channels=model_cfgs_small['out_channels'],
                embed_out_indice=model_cfgs_small['embed_out_indice'],
                decode_out_indices=model_cfgs_small['decode_out_indices'],
                depths=4,
                num_heads=model_cfgs_small['num_heads'],
                c2t_stride=model_cfgs_small['c2t_stride'],
                drop_path_rate=0.1,
            )
            self.seg_head = SimpleHead(
        ...
        if type == 'tiny':
            self.backbone = Topformer(
                cfgs=model_cfgs_tiny['cfg'],
                channels=model_cfgs_tiny['channels'],
                out_channels=model_cfgs_tiny['out_channels'],
                embed_out_indice=model_cfgs_tiny['embed_out_indice'],
                decode_out_indices=model_cfgs_tiny['decode_out_indices'],
                depths=4,
                num_heads=model_cfgs_tiny['num_heads'],
                c2t_stride=model_cfgs_tiny['c2t_stride'],
                drop_path_rate=0.1,
            )
            self.seg_head = SimpleHead(
        ...
        if type == 'base':
            self.backbone = Topformer(
                cfgs=model_cfgs_base['cfg'],
                channels=model_cfgs_base['channels'],
                out_channels=model_cfgs_base['out_channels'],
                embed_out_indice=model_cfgs_base['embed_out_indice'],
                decode_out_indices=model_cfgs_base['decode_out_indices'],
                depths=4,
                num_heads=model_cfgs_base['num_heads'],
                c2t_stride=model_cfgs_base['c2t_stride'],
                drop_path_rate=0.1,
                init_cfg=dict(
                    type='Pretrained', checkpoint=None)
                # init_cfg=dict(
                #     type='Pretrained', checkpoint='/data/cj/project/unext_pro/models/Topformer/classification/topformer-B-224-75.3.pth')
            )
            self.seg_head = SimpleHead(
