...
class CLS(nn.Module):
    def __init__(self,
        ...
        self.in_index = kwargs.get("in_index")
        self.channels = kwargs.get("channels")
        self.norm_cfg = kwargs.get("norm_cfg")
...
class afformer_base(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        self.backbone = AFFormer(
            img_size=224,
            num_stages=4,
            num_path=[1, 1, 1, 1],
            num_layers=[1, 2, 6, 2],
            embed_dims=[32, 96, 176, 216],
            mlp_ratios=[2, 2, 2, 2],
            num_heads=[8, 8, 8, 8],strides = [4, 2, 2, 2])
        self.decode = CLS(
            in_channels=[216],
            in_index=[3],
            channels=256,
